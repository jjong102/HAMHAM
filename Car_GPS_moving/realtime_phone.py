...
class RouteBgAndPoseViz(Node):
    ...
    + (옵션) /current_pose 화살표 + yaw 화살표(라벨은 /current_yaw 0..360° 그대로)
    퍼블리시: /viz/route_markers (MarkerArray)

    ※ /current_yaw는 '북=0°, CCW+'(deg)로 가정합니다.
    """
    def __init__(self):
        super().__init__('route_bg_and_pose_viz')

        # ------------ 파라미터 ------------
        self.declare_parameter('route_csv', '~/IMU/second/gps_latlon__1.csv')  # 초기 CSV (옵션)
        self.declare_parameter('set_path_topic', '/set_path_file')   # 경로 교체 알림 토픽(String)
        ...
        self.declare_parameter('yaw_offset_deg', 0.0)          # 화살표 각도 오프셋
        ...
        self.set_path_topic = self.get_parameter('set_path_topic').get_parameter_value().string_value
        ...
        if not self.route_ll and self.route_csv:
            self.get_logger().warn(f"초기 route_csv 읽기 실패 또는 비어있음: {self.route_csv}")
        ...
        # yaw: /current_yaw(북=0, CCW+, 0..360°)
        self.last_yaw_heading_deg = None
        ...
        # 경로 교체 알림 구독: String(data=CSV 경로)
        self.sub_setp = self.create_subscription(String, self.set_path_topic, self.cb_set_path_file, 10)
        ...
        self.get_logger().info(
            ...
            f"yaw_topic='{self.yaw_topic}', origin_mode='{self.origin_mode}', set_path_topic='{self.set_path_topic}'"
        )

    # --------- 경로 교체 콜백 ---------
    def cb_set_path_file(self, msg: String):
        new_path = os.path.expanduser(msg.data.strip())
        if not new_path:
            self.get_logger().warn("[set_path] 빈 경로 수신 → 무시")
            return
        if not os.path.exists(new_path):
            self.get_logger().error(f"[set_path] 파일이 없음: {new_path} → 이전 경로 유지")
            return

        new_ll = self._load_route_latlon(new_path)
        if not new_ll:
            self.get_logger().error(f"[set_path] CSV 읽기 실패 또는 포맷 빈약: {new_path} → 이전 경로 유지")
            return

        self.route_csv = new_path
        self.route_ll = new_ll

        # origin_mode가 csv_first면 새 경로 첫 점으로 원점 갱신
        if self.origin_mode == 'csv_first':
            self.lat0, self.lon0 = self.route_ll[0]
            self.origin_set = True
            self.get_logger().info(f"[set_path] origin(csv_first) 갱신: ({self.lat0:.6f},{self.lon0:.6f})")

        # fix/manual 모드면 기존 원점 유지 (fix는 아직 미설정일 수 있음)
        self._project_route_if_ready()
        if not self.route_xy_ready:
            # origin이 아직 없으면(예: fix 모드에서 GPS 미도착) 투영 대기
            self.get_logger().info("[set_path] origin 미설정 → GPS fix/수동 원점 대기 후 경로 표시 예정")
        else:
            self.get_logger().info(f"[set_path] 경로 교체 완료: {os.path.basename(self.route_csv)} "
                                   f"(pts={len(self.route_xy)})")

    # --------- yaw 타입/구독 관리 ---------
    def _setup_yaw_subscription(self):
        if self._yaw_sub is not None:
            try: self.destroy_subscription(self._yaw_sub)
        ...
        if force in ("float64", "float32"):
            self._create_yaw_sub(force); return

        types_map = dict(self.get_topic_names_and_types())
        types = types_map.get(self.yaw_topic, [])
        if 'std_msgs/msg/Float64' in types:   self._create_yaw_sub('float64')
        elif 'std_msgs/msg/Float32' in types: self._create_yaw_sub('float32')
        else:
            self._create_yaw_sub('float64')
            self.get_logger().warn(
                f"[yaw] {self.yaw_topic} 타입을 못 찾음 → 임시 Float64 구독. 필요 시 자동 재탐지."
            )

        if 'std_msgs/msg/Float64' in types and 'std_msgs/msg/Float32' in types:
            self.get_logger().warn(
                f"[yaw] {self.yaw_topic} 에 Float32/Float64 동시 퍼블리시 감지. (현재 {self._yaw_type}로 수신)"
            )
    ...
    def _maybe_resub_yaw(self):
        ...
        types = types_map.get(self.yaw_topic, [])
        target = None
        if 'std_msgs/msg/Float64' in types: target = 'float64'
        elif 'std_msgs/msg/Float32' in types: target = 'float32'
    ...
    def cb_pose(self, msg: PoseStamped):
        ...
        if (self.pose_frame_id != self.frame_id) and (not self.pose_frame_warned):
            ...
            self.get_logger().warn(
                ...
                f"둘 중 하나로 맞추거나 TF 제공 필요. "
                f"빠른 해결: `--ros-args -p frame_id:={self.pose_frame_id}` 로 이 노드와 RViz Fixed Frame을 "
                f"'{self.pose_frame_id}'로 맞추세요."
            )

    # --------- yaw 처리 ---------
    def cb_yaw64(self, msg: Float64): self._handle_yaw(float(msg.data), "Float64")
    def cb_yaw32(self, msg: Float32): self._handle_yaw(float(msg.data), "Float32")

    def _handle_yaw(self, raw_deg: float, src: str):
        hdg = float(raw_deg) % 360.0
        self.last_yaw_heading_deg = hdg
        self._last_yaw_stamp = time.time()
        if not self._yaw_logged_once:
            self.get_logger().info(f"[yaw:{src}] heading(0..360, N=0,CCW+)={hdg:.1f}°")
    ...
    def _build_markers(self):
        ...
        # (C) yaw 화살표
        if (self.last_yaw_heading_deg is not None) and (self.last_fix_xy is not None):
            heading_rad = math.radians(self.last_yaw_heading_deg + self.yaw_offset_deg)  # 북=0°, CCW+
            dx = self.yaw_arrow_len * -math.sin(heading_rad) # ENU: X(동)=sin, Y(북)=cos
            dy = self.yaw_arrow_len * math.cos(heading_rad)
            gx, gy = self.last_fix_xy
            ...
            txt.text = f"heading (N=0,CCW+) {self.last_yaw_heading_deg:.1f}°"
